...
class RandomTwoComponentDiploidReproducer(object):
    ...
    @staticmethod
    def generate_gametes(model, n_gametes, prob_crossover=0.5):
    ...
    def cross(male_model,
        ...
        male_init_states, male_init_pts, male_params \
            = RandomTwoComponentDiploidReproducer.generate_gametes(male_model, n_gametes, prob_crossover)
        female_init_states, female_init_pts, female_params \
            = RandomTwoComponentDiploidReproducer.generate_gametes(female_model, n_gametes, prob_crossover)
...
def _process_cross_group(male_model, female_model, n_progenies, n_gametes,
    ...
    子ごとのモデルと画像を返す。ワーカープロセス内で実行される。"""

    pa_model, ma_model = RandomTwoComponentDiploidReproducer.cross(
        male_model=male_model, female_model=female_model,
        n_progenies=n_progenies, n_gametes=n_gametes,
        prob_crossover=prob_crossover, autosomal=autosomal, device=device)

    model = diploid_model_class(paternal_model=pa_model, maternal_model=ma_model,
    ...
    for k in range(n_progenies):
        ...
        progeny_pa_model = haploid_model_class(
            initializer=pa_init, params=pa_model.params[None, k, :],
            width=model.width, height=model.height, dx=model.dx, device=device)

        ma_init = haploid_initializer_class(
            init_states=ma_model.initializer.init_states[None, k, :],
            init_pts=ma_model.initializer.init_pts[None, k, :, :])
        progeny_ma_model = haploid_model_class(
            initializer=ma_init, params=ma_model.params[None, k, :],
            width=model.width, height=model.height, dx=model.dx, device=device)

        progeny_model = diploid_model_class(
            paternal_model=progeny_pa_model, maternal_model=progeny_ma_model,
            alpha=alpha, beta=beta, device=device)

        progenies.append({"MODEL": progeny_model, "MORPH": img_ladybird, "PATTERN": img_pattern})
...
class FitnessSelectingDiploidReproducer(RandomTwoComponentDiploidReproducer):
    ...
    def evolve(self, n_generations=None, verbose=None):
        ...
        # 世代0(初期集団)は親クラスと同じ形でOK。fitnessだけ計算しておく。
        prev_gen = self._population_dicts[0]
        self._fitness_log = []   # 履歴として持つのは「fitness値」だけ(モデルや画像は持たない)

        for i in range(1, n_generations):
            t_beg = time.time()

            # 1. n_cross個のグループを並列実行
            ...
            # 3. 選択(小さい方が良い、という前提。EvoSearchのfitnessと同じ向き)
            all_progenies.sort(key=lambda p: p["FITNESS"])
            selected = all_progenies[: self._pop_size]

            # 4. 保存 + 「直前世代だけ」を次の親候補として保持(履歴は溜めない)
            new_gen = []
            for idx, p in enumerate(selected):
                str_id = "generation-%d_model-%d" % (i, idx + 1)
                new_gen.append({"ID": str_id, "MODEL": p["MODEL"], "FITNESS": p["FITNESS"]})
                self._fitness_log.append((i, str_id, p["FITNESS"]))

                if self._dpath_output:
                    # モデルJSON・画像をディスクに保存(前回話した通り、保存したらメモリ上には残さない)
                    ...

            prev_gen = new_gen   # ← 前世代を上書き。古い世代はGCで解放される
