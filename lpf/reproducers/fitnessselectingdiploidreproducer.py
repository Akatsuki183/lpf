...
class FitnessSelectingDiploidReproducer(RandomTwoComponentDiploidReproducer):
    ...
    def evolve(self, n_generations=None, verbose=None):
        ...
        # spawn: 親プロセスがCUDA(torch等)を使っていても、
        # ワーカープロセスにその状態を引き継がず安全に起動できる。
        # プールは世代ループの外で1回だけ作り、世代をまたいで使い回す
        # (spawnはforkよりプロセス起動が重いので、毎世代作り直すと無駄が大きい)。
        ctx = multiprocessing.get_context("spawn")
 
        with ProcessPoolExecutor(max_workers=self._n_procs, mp_context=ctx) as executor:
            for i in range(1, n_generations):
                ...
                # --- ① 親ペアを先に確定させてから、n_cross個のグループを並列実行 ---
                ...
                # --- ② fitness評価(target画像との比較。軽いのでメインプロセスでOK) ---
                ...
                if verbose:
                    print(
                        ...
                        "(total %.3f sec. | ①並列計算 %.3f / ②fitness評価 %.3f / "
                        "③選択 %.3f / ④保存 %.3f)"
                        % (i, selected[0]["FITNESS"], t4 - t_beg,
                           t1 - t_beg, t2 - t1, t3 - t2, t4 - t3)
                    )
